...
class TBD(cog.cogModule):
    ...
    def answer_by_programs(self, features, programs):
        ...
        for n in range(batch_size): #[:4]
            ...
            result, result2 = self.run_program(output, program)
            results.append(result)
            results2.append(result2)

        return results, results2 #let's compare accuracies
    ...
    def run_program(self, features, program):
        ...
        eval_link, left, inheritance_set = self.return_prog(self.atomspace, rev_prog)
        ...
        for inh in inheritance_set:
            for atom in inh.get_out():
                if atom.type == types.VariableNode:
                    varlist.append(atom)
        variable_list = self.atomspace.add_link(types.VariableList, varlist)
        
        list_link = self.atomspace.add_link(types.ListLink, varlist + [eval_link])
        conj = self.atomspace.add_link(types.AndLink, [*inheritance_set, eval_link])
        bind_link = self.atomspace.add_link(types.BindLink, [variable_list, conj, list_link])
        ...
        print ("executing the bind_link to get the result")
    ...
    def return_prog(self, atomspace, commands, inheritance_set=None):
        """
        ...
        if current.startswith('query'):
            ...
            sub_prog, left, inh = self.return_prog(atomspace, rest)
            inheritance_set |= inh
            ...
            result = atomspace.add_link(types.ExecutionOutputLink, [schema, lst])
            ...
            lst = atomspace.add_link(types.ListLink, [filter_type_atom,
                                                      filter_arg_atom,
                                                      sub_prog])
            exec_link = atomspace.add_link(types.ExecutionOutputLink, [schema, lst])
            ...
            lst = atomspace.add_link(types.ListLink, [atomspace.add_node(types.ConceptNode, relate_arg),
                                                      sub_prog])
            exec_out_link =  atomspace.add_link(types.ExecutionOutputLink, [schema, lst])
            return exec_out_link, left, inheritance_set
            ...
            lst = atomspace.add_link(types.ListLink, [atomspace.add_node(types.ConceptNode, same_arg),
                                                      sub_prog])
            exec_out_link = atomspace.add_link(types.ExecutionOutputLink, [schema, lst])
            return exec_out_link, left, inheritance_set

        elif current.startswith('intersect'):
            sub_prog0, left, inh = self.return_prog(atomspace, rest)
            inheritance_set |= inh
            sub_prog1, right, inh = self.return_prog(atomspace, left)
            inheritance_set |= inh
            
            schema =  atomspace.add_node(types.GroundedSchemaNode, "py:intersect")
            lst = atomspace.add_link(types.ListLink, [sub_prog0,
                                                      sub_prog1])
            exec_out_link = atomspace.add_link(types.ExecutionOutputLink, [schema, lst])
            return exec_out_link, right, inheritance_set


        elif current == '<START>':
            return self.return_prog(atomspace, rest)
    ...
    def return_prog2(self, atomspace, commands, inheritance_set2=None):
        ...
        if current.startswith('query'):
            ...
            exec_result_filter = execute_atom(self.atomspace, exec_link)
            ...
            exec_result_relate = execute_atom(self.atomspace, exec_out_link)
            ...
            lst = atomspace.add_link(types.ListLink, [atomspace.add_node(types.ConceptNode, same_arg),
                                                      sub_prog])
            exec_out_link = atomspace.add_link(types.ExecutionOutputLink, [schema, lst])
            exec_result_same = execute_atom(self.atomspace, exec_out_link)
            ...
            lst = atomspace.add_link(types.ListLink, [sub_prog0,
                                                      sub_prog1])
            exec_out_link = atomspace.add_link(types.ExecutionOutputLink, [schema, lst])
            exec_result_intersect = execute_atom(self.atomspace, exec_out_link)
...
def main():
    ...
    for i, batch in enumerate(tqdm(loader)):
        ...
        results, results2 = tbd.answer_by_programs(feats, programs)
